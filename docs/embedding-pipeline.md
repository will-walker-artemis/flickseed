...
flowchart TB
    subgraph inputs["Input Files"]
        films["data/raw/films.csv<br><i>~150 films from finalize</i>"]
    ...
    subgraph views["Four Parallel Views"]
        direction TB
    ...
    class films,keywords,credits,corpus,notes input
    class v_overview,v_keywords,v_crew,v_notes view
...
### 4. Notes — personal signal (384-dim, optional)
...
|---|---|---|---|
| `data/derived/embeddings.parquet` | Parquet (pandas + pyarrow) | One row per film: `tmdb_id`, `title`, `vector` (final composed), plus `vector_overview`, `vector_keywords`, `vector_crew`, `vector_notes` for debugging | committed |
...
**Config:** view weights = {overview: 1.0, keywords: 0.8, crew: 0.8, notes: 1.0}
...
are all 1980s sci-fi action films, the text overview is dominating. Raise `crew`
and `keywords` weights (try `1.2`), lower `overview` (try `0.7`), re-embed, and
check again.
...
**Upstream:** `get_films.py --mode finalize` produces `films.csv` with keywords
and credits baked in, `enrich/` parses the CSV into JSON, `corpus/` writes
per-film markdown.
